...
class WeerplazaParser:
    ...
    def extract_data(self):
        ...
        try:
            ...
            if hourly_container:
                for hour_div in hourly_container.find_all("div", class_="hour"):
                    ...
                    time_val = (
                        head.find_all("div")[-1].get_text(strip=True)
                        if head
                        else "--:--"
                    )
                    wx_div = hour_div.find("div", class_="wx")
                    icon_match = (
                        re.search(r"url\(['\"]?(.*?)['\"]?\)", wx_div.get("style", ""))
                        if wx_div
                        else None
                    )
        ...
        try:
            ...
            if daily_section:
                ...
                for cell in all_cells:
                    ...
                    head = cell.find("div", class_="show-large") or cell.find(
                        "div", class_="hide-large"
                    )
                    if head:
                        ...
                        day_map[day_id]["date_short"] = (
                            f"{day_name} {date_txt}".strip().capitalize()
                        )
                    ...
                    if wx:
                        ...
                        day_map[day_id]["icon"] = (
                            icon_match.group(1) if icon_match else ""
                        )
                        day_map[day_id]["description_icon_title"] = wx.get(
                            "title", "Verwachting"
                        )
        ...
        while len(scraped_data["daily_forecast_summary"]) < 4:
            scraped_data["daily_forecast_summary"].append(
                {
                    "date_short": "-",
                    "description_icon_title": "-",
                    "temp_high": "-",
