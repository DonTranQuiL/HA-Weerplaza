...
def mock_entry():
    return MockConfigEntry(
        domain=DOMAIN,
        data={
            "instance_name": "Eygelshoven",
            "location_path": "nederland/eygelshoven/9314",
        },
...
async def test_coordinator_successful_scrape(
    mock_session_cls, hass: HomeAssistant, mock_entry
):
    ...
    with (
        ...
        patch("custom_components.weerplaza.coordinator.asyncio.sleep"),
    ):
...
@pytest.mark.asyncio
@patch("custom_components.weerplaza.coordinator.aiohttp.ClientSession")
async def test_coordinator_server_error_fallback(
    mock_session_cls, hass: HomeAssistant, mock_entry
):
...
@pytest.mark.asyncio
@patch("custom_components.weerplaza.coordinator.aiohttp.ClientSession")
async def test_coordinator_total_failure(
    mock_session_cls, hass: HomeAssistant, mock_entry
):
