...
from custom_components.weerplaza import (
    async_setup,
    async_setup_entry,
    async_unload_entry,
    async_reload_entry,
...
def mock_dependencies():
    ...
    with (
        patch("custom_components.weerplaza.PersistentCache") as mock_cache_cls,
...
async def test_setup_entry_no_cache(hass: HomeAssistant, mock_dependencies):
    ...
    entry = MockConfigEntry(
        domain=DOMAIN, data={"instance_name": "Test", "location_path": "nl/test"}
    )
...
async def test_setup_entry_with_cache(hass: HomeAssistant, mock_dependencies):
    ...
    with patch(
        ...
        mock_coord.async_set_updated_data.assert_called_once_with(
